...
// Seed data from docs/Demo workflows.md ("Seed data required" and "Demo logins"). Names, serials and
// phone numbers are fictional. Dates of the named units follow their serials; the filler units are
// placed relative to `today`, so "expiring soon" and "active" still hold whenever the demo runs.
// Admin -> Simulate -> Reset demo data calls createSeed() again.

/** Shared password of every demo account. Lives on the server only; the sign-in page gets it from /auth/demo-accounts. */
export const DEMO_PASSWORD = "Demo#2026";

/** Accounts offered on the sign-in page (docs/Demo workflows.md, "Demo logins"). */
export const DEMO_ACCOUNTS = [
  ...
  { email: "dealer.coolair@demo.wms", label: "Dealer: CoolAir Traders, Pune" },
  {
    ...
    label: "Distributor: NorthStar Distribution",
  },
...
export function createSeed(today: IsoDate = todayIso()): DemoState {
  ...
  const state: DemoState = {
    ...
    models: [
      {
        ...
        code: "AER-SPL15",
        brandId: "b-aer",
        name: "Aeris Split 1.5 TR",
        capacity: "1.5 TR",
        type: "Split",
        parts: AERIS_TEMPLATE,
      },
      {
        id: "m-aer-spl18",
        code: "AER-SPL18",
        brandId: "b-aer",
        name: "Aeris Split 1.8 TR",
        capacity: "1.8 TR",
        type: "Split",
        parts: AERIS_TEMPLATE,
      },
    ...
    distributors: [
      { id: "dist-northstar", name: "NorthStar Distribution", city: "Pune" },
    ],
    dealers: [
      {
        id: "d-coolair",
    ...
    users: [
      ...
      },
      {
        id: "u-breeze",
        name: "Breeze Point",
        email: "dealer.breeze@demo.wms",
        role: "dealer",
        dealerId: "d-breeze",
      },
      {
        id: "u-arctic",
        name: "Arctic Home Solutions",
        email: "dealer.arctic@demo.wms",
        role: "dealer",
        dealerId: "d-arctic",
      },
      {
  ...
  const dealerUser = (dealerId: string) =>
    state.users.find((u) => u.dealerId === dealerId)!;
  ...
  function registerUnit(spec: {
    ...
    const registrationId = nextId(state, "REG");
    state.registrations.push({
      ...
      attachmentIds: [],
      submittedBy:
        spec.channel === "ERP" || spec.channel === "EMAIL"
          ? "system"
          : submitter.id,
      submittedByName:
        spec.channel === "ERP"
    ...
    const unit: Unit = {
      ...
      history: [
        {
          at: ts(spec.installDate, "15:00:00"),
          type: "registered",
    ...
    history: [
      {
        at: ts(daysAgo(10)),
        type: "note",
        byName: "CoolAir Traders",
        text: "Sold, QR label printed",
      },
    ],
  });
  ...
  // --- Filler units ------------------------------------------------------------------------------
  // CoolAir Traders (Pune)
  registerUnit({
    serial: "AER-SPL18-251120",
    modelId: "m-aer-spl18",
    dealerId: "d-coolair",
    customerId: "c-aj",
  ...
  registerUnit({
    serial: "AER-SPL15-250912",
    modelId: "m-aer-spl15",
    dealerId: "d-coolair",
    customerId: "c-kk",
    installDate: daysAgo(355),
  ...
  registerUnit({
    ...
    dealerId: "d-breeze",
    customerId: "c-sc",
    installDate: daysAgo(580),
    location: "Sunrise Clinic, Nashik: reception",
    channel: "ERP",
  });
  registerUnit({
    serial: "AER-SPL18-250805",
    modelId: "m-aer-spl18",
    dealerId: "d-breeze",
    customerId: "c-np",
  ...
  registerUnit({
    serial: "POL-SPL12-230910",
    modelId: "m-pol-spl12",
    dealerId: "d-breeze",
    customerId: "c-rg",
    installDate: daysAgo(1100),
  ...
  registerUnit({
    ...
    dealerId: "d-arctic",
    customerId: "c-ho",
    installDate: daysAgo(990),
    location: "Harbour Offices, Mumbai: 3rd floor",
    channel: "ERP",
  });
  registerUnit({
    serial: "POL-SPL12-250720",
    modelId: "m-pol-spl12",
    dealerId: "d-arctic",
    customerId: "c-vr",
  ...
  function history(spec: {
    ...
    const complaintId = nextId(state, "CMP");
    const raisedBy =
      spec.source === "DEALER"
        ? dealerUser(unit.dealerId!)
        : { id: "u-admin", name: "WMS office admin" };
    const serviceRequestId = nextId(state, "SR");
      ...
      history: [
        {
          at: ts(raised),
          status: "NEW",
          byName: spec.source === "CUSTOMER" ? customer.name : raisedBy.name,
        },
        {
          at: ts(raised, "11:00:00"),
          status: "WITH_SERVICE",
    ...
    const old = unit.parts.find(
      (p) => p.partType === line.partType && !p.replacedAt,
    );
    ...
    const partsReplaced = [
      { partType: line.partType, oldSerial: old?.serial, newSerial },
    ];
    ...
    const claimId = nextId(state, "CLM");
    const claim: Claim = {
      id: claimId,
      complaintId,
      unitSerial: unit.serial,
      ...
      history: [
        { at: ts(fixed, "16:10:00"), status: "DRAFT", byName: "System" },
      ],
    };
    ...
    if (
      spec.claim.status === "APPROVED" ||
      spec.claim.status === "PAID" ||
      spec.claim.status === "REJECTED"
    ) {
      const decided = addDaysIso(fixed, 4);
      const decision =
        spec.claim.status === "REJECTED" ? "REJECTED" : "APPROVED";
      claim.history.push({
        at: ts(decided),
        status: decision,
        byName: "OEM",
        text: spec.claim.rejectReason,
      });
      message({
        system: "OEM",
        direction: "IN",
  ...
  history({
    ...
    claim: {
      status: "REJECTED",
      amount: 18200,
      rejectReason: "Installation not done by an authorised installer.",
    },
  });
  history({
    serial: "AER-SPL18-240110",
  ...
  const erpRegistration = state.registrations.find(
    (r) => r.serial === "KEL-CAS30-240220",
  )!;
  message({
    system: "ERP",
    direction: "IN",
