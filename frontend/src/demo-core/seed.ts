...
export function createSeed(today: IsoDate = todayIso()): DemoState {
  ...
  function registerUnit(spec: {
    ...
    const submitter =
      spec.channel === "PORTAL" ? { id: customer.id, name: customer.name } : dealerUser(spec.dealerId);
  ...
  state.units.push({
    ...
    attachmentIds: [],
    history: [
      { at: ts(daysAgo(10)), type: "note", byName: "CoolAir Traders", text: "Sold, QR label printed" },
    ],
  });
  ...
  function message(
    ...
    state.integrations.push({ id: nextId(state, "MSG"), attempts: 1, createdAt: at, updatedAt: at, ...rest });
  }
  ...
  function history(spec: {
    ...
    const raisedBy =
      spec.source === "DEALER" ? dealerUser(unit.dealerId!) : { id: "u-admin", name: "WMS office admin" };
