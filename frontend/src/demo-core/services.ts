...
// Scoped read services behind every demo endpoint. Transport adapters (Express in demo-server/, MSW in
// src/mocks/) only parse the request, call one of these and serialise the result or the ServiceError.
...
function paginate<T>(items: T[], { page = 1, pageSize = 25 }: ListQuery): Paginated<T> {
...
function sortRows<T>(items: T[], sort: string | undefined, fallback: string): T[] {
...
const dealerName = (state: DemoState, id?: string) => state.dealers.find((d) => d.id === id)?.name;
const customerName = (state: DemoState, id?: string) => state.customers.find((c) => c.id === id)?.name;
const brandName = (state: DemoState, id: string) => state.brands.find((b) => b.id === id)?.name ?? id;

// ---- auth ----------------------------------------------------------------------------------------

export function authenticate(state: DemoState, email: string, password: string): User {
...
function findUnit(ctx: Ctx, serial: string): Unit {
  ...
  if (!unit || !canSee(ctx.user, unit, ctx.state.dealers)) throw notFound("Unit");
  return unit;
}
...
function toRegistrationView(ctx: Ctx, r: Registration): RegistrationView {
  ...
  return {
    ...
    duplicateOf:
      duplicate && ctx.user.role === "admin" ? toUnitView(ctx.state, duplicate, ctx.today) : undefined,
  };
}
...
export function listRegistrations(
  ...
  const rows = ctx.state.registrations.filter(
    (r) =>
      ...
      matches(query.q, r.serial, r.customer.name, r.modelCode, dealerName(ctx.state, r.dealerId)),
  );
  return paginate(
    sortRows(rows, query.sort, "-submittedAt").map((r) => toRegistrationView(ctx, r)),
    query,
  );
}
...
function toComplaintView(ctx: Ctx, c: Complaint): ComplaintView {
  ...
  return {
    ...
    claimStatus: claim && canSeeClaim(ctx.user, claim, ctx.state.dealers) ? claim.status : undefined,
  };
}

export function listComplaints(
  ctx: Ctx,
  query: ListQuery & { status?: ComplaintStatus; source?: ComplaintSource } = {},
  ...
  const rows = ctx.state.complaints
    ...
    .filter((c) => matches(query.q, c.id, c.unitSerial, c.customerName, c.dealerName));
...
export function getComplaint(ctx: Ctx, id: string): ComplaintView {
  ...
  if (!c || !canSee(ctx.user, c, ctx.state.dealers)) throw notFound("Complaint");
...
export function listClaims(
  ...
  const rows = ctx.state.claims
    ...
    .filter((c) => matches(query.q, c.id, c.unitSerial, c.rmaNumber, c.brandName));
  return paginate(sortRows(rows, query.sort, "-createdAt"), query);
}

export function getClaim(ctx: Ctx, id: string): ClaimView {
  const c = ctx.state.claims.find((x) => x.id === id);
  if (!c || !canSeeClaim(ctx.user, c, ctx.state.dealers)) throw notFound("Claim");
  return toClaimView(ctx, c);
}

export function claimCounts(ctx: Ctx): Record<ClaimStatus, number> {
  const counts: Record<ClaimStatus, number> = { DRAFT: 0, SUBMITTED: 0, APPROVED: 0, PAID: 0, REJECTED: 0 };
  for (const c of ctx.state.claims) if (canSeeClaim(ctx.user, c, ctx.state.dealers)) counts[c.status] += 1;
  return counts;
}
...
export const listModels = (ctx: Ctx): ModelView[] =>
  ctx.state.models.map((m) => ({ ...m, brandName: brandName(ctx.state, m.brandId) }));
...
const DASHBOARD_CHANNELS: ChannelCount["channel"][] = ["DEALER", "PORTAL", "EMAIL", "ERP"];
...
export function dashboardSummary(ctx: Ctx): DashboardSummary {
  ...
  const registered = units.filter((u) => u.status !== "PENDING");
  const complaints = state.complaints.filter((c) => canSee(user, c, state.dealers));
  const claims = state.claims.filter((c) => canSeeClaim(user, c, state.dealers));
  const registrations = state.registrations.filter((r) => canSee(user, r, state.dealers));
  const countStatus = (s: WarrantyStatus) => registered.filter((u) => u.status === s).length;
  ...
  const dealers: DealerStats[] = listDealers(ctx).map((d) => ({
    ...
    openComplaints: complaints.filter((c) => c.dealerId === d.id && c.status !== "RESOLVED").length,
  }));
  return {
    role: user.role,
...
export function listIntegrations(
  ctx: Ctx,
  query: ListQuery & { system?: string; direction?: string; status?: string } = {},
) {
  requireRole(ctx, "admin");
  ...
  return paginate(sortRows<IntegrationMessage>(rows, query.sort, "-createdAt"), query);
}
...
export function getAttachment(ctx: Ctx, id: string): Attachment {
  ...
  const visible =
    ...
    state.registrations.some((r) => referencedBy(r.attachmentIds) && canSee(user, r, state.dealers)) ||
    state.complaints.some((c) => referencedBy(c.attachmentIds) && canSee(user, c, state.dealers)) ||
    state.units.some((u) => referencedBy(u.attachmentIds) && canSee(user, u, state.dealers)) ||
    state.jobResults.some((j) => {
      const complaint = state.complaints.find((c) => c.id === j.complaintId);
      return referencedBy(j.photoIds) && !!complaint && canSee(user, complaint, state.dealers);
    });
