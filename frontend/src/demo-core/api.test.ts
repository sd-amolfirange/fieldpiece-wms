import type { Paginated, UnitView } from "../domain";
import { createSessionStore, dispatch, type DemoDb, type DemoRequest } from "./api";
...
function setup() {
  ...
  const login = (email: string) => {
    const res = call({ method: "POST", path: "/auth/login", body: { email, password: DEMO_PASSWORD } });
    expect(res.status).toBe(200);
    return (res.body as { accessToken: string }).accessToken;
  };
  const units = (token: string, query: DemoRequest["query"] = {}) =>
    call({ path: "/units", accessToken: token, query: { pageSize: "100", ...query } })
...
describe("seed", () => {
  ...
  const unit = (serial: string) => state.units.find((u) => u.serial === serial)!;
  ...
  it("has the named units from the workflows doc", () => {
    ...
    expect(unit("AER-SPL15-210311").parts.find((p) => p.partType === "COMPRESSOR")?.warrantyEnd).toBe(
      "2031-03-11",
    );
    expect(unit("AER-SPL18-230502").history.some((e) => /unauthorised repair/i.test(e.text ?? ""))).toBe(
      true,
    );
  });
  ...
  it("links NorthStar to CoolAir Traders and Breeze Point", () => {
    expect(state.dealers.filter((d) => d.distributorId === "dist-northstar").map((d) => d.name)).toEqual([
      "CoolAir Traders",
      "Breeze Point",
    ]);
  });
  ...
  it("only raises claims for claimable complaints", () => {
    for (const claim of state.claims) {
      const complaint = state.complaints.find((c) => c.id === claim.complaintId)!;
...
describe("demo API auth", () => {
  ...
  it("requires a token and restores the session from the refresh cookie", () => {
    ...
    const refresh = call({ method: "POST", path: "/auth/refresh", refreshToken: login.setRefreshToken });
    expect(refresh.status).toBe(200);
    expect((refresh.body as { user: { role: string } }).user.role).toBe("admin");
    const out = call({ method: "POST", path: "/auth/logout", refreshToken: login.setRefreshToken });
    expect(out.clearRefreshToken).toBe(true);
    expect(call({ method: "POST", path: "/auth/refresh", refreshToken: login.setRefreshToken }).status).toBe(
      401,
    );
  });
});
...
describe("demo API scoping", () => {
  it("scopes units per role", () => {
    ...
    expect(coolair.items.every((u) => u.dealerName === "CoolAir Traders")).toBe(true);
    const customer = units(login("customer.rk@demo.wms"));
    expect(customer.items.map((u) => u.serial).sort()).toEqual(["AER-SPL15-210311", "AER-SPL18-230502"]);
  });

  it("hides other dealers' records behind 404", () => {
    const { call, login } = setup();
    const token = login("dealer.coolair@demo.wms");
    expect(call({ path: "/units/KEL-CAS30-240220", accessToken: token }).status).toBe(404);
    expect(call({ path: "/units/AER-SPL15-210311", accessToken: token }).status).toBe(200);
  });

  it("lets the distributor filter by dealer", () => {
    const { login, units } = setup();
    const res = units(login("dist.northstar@demo.wms"), { dealerId: "d-breeze" });
    expect(res.total).toBe(4);
  });

  it("computes part-wise status in unit views", () => {
    const { call, login } = setup();
    const res = call({ path: "/units/AER-SPL15-210311", accessToken: login("customer.rk@demo.wms") });
  ...
  it("keeps claims, integrations and admin data away from customers and dealers", () => {
    ...
    expect(call({ path: "/integrations", accessToken: dealer }).status).toBe(403);
    expect(call({ path: "/admin/org", accessToken: dealer }).status).toBe(403);
    expect(call({ method: "POST", path: "/simulate/reset", accessToken: dealer }).status).toBe(403);
    const claims = call({ path: "/claims", accessToken: dealer, query: { pageSize: "100" } })
      .body as Paginated<{
      dealerId: string;
    }>;
  ...
  it("returns role-specific dashboards", () => {
    const { call, login } = setup();
    const admin = call({ path: "/dashboard/summary", accessToken: login("admin@demo.wms") }).body as {
      role: string;
      units: number;
      registrationsByChannel: { channel: string; count: number }[];
    };
    expect(admin.role).toBe("admin");
    expect(admin.units).toBe(14);
    expect(admin.registrationsByChannel.map((c) => c.channel)).toEqual(["DEALER", "PORTAL", "EMAIL", "ERP"]);
    const dist = call({ path: "/dashboard/summary", accessToken: login("dist.northstar@demo.wms") }).body as {
      dealers: { dealerName: string }[];
    };
    expect(dist.dealers.map((d) => d.dealerName)).toEqual(["CoolAir Traders", "Breeze Point"]);
    const dealer = call({ path: "/dashboard/summary", accessToken: login("dealer.coolair@demo.wms") })
      .body as {
      dealers: unknown[];
    };
  ...
  it("resets demo data for the admin", () => {
    ...
    db.state.units = [];
    const res = call({ method: "POST", path: "/simulate/reset", accessToken: login("admin@demo.wms") });
    expect(res.status).toBe(200);
...
describe("attachments", () => {
  it("lets only the uploader, admins and people who can see the linked record open a file", () => {
    ...
    expect(() => getAttachment(ctxFor("u-breeze"), file.id)).toThrow(ServiceError);

    state.units.find((u) => u.serial === "AER-SPL15-210311")!.attachmentIds.push(file.id);
  ...
  it("refuses unsupported and oversized files", () => {
    ...
    expect(() =>
      addAttachment(ctx, { name: "a.exe", mime: "application/x-msdownload", size: 10 }, String),
    ).toThrow(/photo, video or PDF/);
    expect(() =>
      addAttachment(ctx, { name: "big.jpg", mime: "image/jpeg", size: 20 * 1024 * 1024 }, String),
    ).toThrow(/15 MB/);
  });
});
